...
if (!Array.isArray(pullRequests) || pullRequests.length === 0) {
  return errorResult(`No pull request found for branch ${branchName}.`)
}
...
if (!(pullRequest?.mergeable)) {
  return errorResult(`Pull request found for branch ${branchName} contains merge conflicts.`)
}
...
function errorResult(errorMessage) {
  if (args.prRequired) {
    ...
    return core.setOutput('results', null)
  }
}
